...
PatternEngine::PatternEngine(IPlatformUtils& pUtils, const int num_leds)
    :   current_pattern{pUtils, num_leds},
        ...
        last_beat_time_micros{pUtils.crono().currentTimeMicros()},
...
PatternEngine::~PatternEngine() {
    if (this->allPatternBlueprints != nullptr) {
        ...
        delete this->allPatternBlueprints[1];
...
void PatternEngine::start(const pattern_engine_config& config) {
    ...
    pUtils.logger().log("Got here");
    this->time_since_last_beat_micros = pUtils.crono().currentTimeMicros();
    this->allPatternBlueprints = (PatternBlueprint**) malloc(sizeof(PatternBlueprint*) * NUM_PATTERNS);

    this->allPatternBlueprints[PatternTypeCast::to_index(PatternType::COLORED_AMPLITUDE)] = new ColoredAmplitude(pUtils, num_leds);
    this->allPatternBlueprints[PatternTypeCast::to_index(PatternType::ALTERNATING_COLORS)] = new AlternatingColors(pUtils, num_leds);
...
const Pattern& PatternEngine::generate_pattern(bool is_beat) {
    ...
    if (is_beat) {
        this->last_beat_time_micros = pUtils.crono().currentTimeMicros();
        this->beats_with_current_pattern++;
    }
    this->time_since_last_beat_micros = pUtils.crono().currentTimeMicros() - this->last_beat_time_micros;

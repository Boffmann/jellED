...
private:
    ...
    void generate_error_pattern(const std::string &error_msg) {
        pUtils.logger().log(error_msg);
        for (int i = 0; i < this->num_leds; i++) {
            this->current_pattern.set_color(rainbow_colors[RED_INDEX], i);
        }
    };

    void generate_rainbow_pattern(const std::string &error_msg) {
        pUtils.logger().log(error_msg);
        for (int i = 0; i < this->num_leds; i++) {
...
public:
    PatternEngine(IPlatformUtils &pUtils, const int num_leds);
    ~PatternEngine();

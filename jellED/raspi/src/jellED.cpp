...
UartFeatures buildUartFeatures(BeatDetector &detector, bool isBeat) {
  ...
  if (isBeat) {
    if (detector.isPeakLow())
      f.beatFlags |= UartFeatures::BEAT_LOW;
    if (detector.isPeakMid())
      f.beatFlags |= UartFeatures::BEAT_MID;
    if (detector.isPeakHigh())
      f.beatFlags |= UartFeatures::BEAT_HIGH;
...
bool initializeMicrophone(UsbMicro &usbMicro) {
  ...
  while (!got_data) {
    ...
    if (got_data) {
      std::cout << "Audio data received! Starting main processing loop."
                << std::endl;
      return true;
    }
    ...
    if (elapsed >= timeout_seconds) {
      std::cout << "Timeout reached (" << timeout_seconds
                << "s). Starting main loop anyway..." << std::endl;
      return false;
    }
...
int main() {
    ...
    while (true) {
...

      {
        ...
        beat_occurred = beat_ready_cv.wait_for(
            lock, std::chrono::milliseconds(UART_THREAD_TIMEOUT_MS),
            [&] { return beat_ready; });
        to_send = latest_features;
        latest_features.beatFlags =
            0; // consumed — don't re-send beat next timeout
        if (beat_occurred) {
      ...
      if (beat_occurred) {
        ...
        auto latency_us = std::chrono::duration_cast<std::chrono::microseconds>(
                              now - detected_time_copy)
                              .count();
        if (latency_us > 1000) {
          std::cout << "[Latency] Beat detect → UART thread: " << latency_us
                    << " µs" << std::endl;
        }
      }
  ...
  Downsampler downsampler(static_cast<float>(usbMicro.getSampleRate()),
  ...
  NoiseGate noiseGate(usbMicro.getSampleRate() / SIGNAL_DOWNSAMPLE_RATIO,
                      config.noiseGateThreshold);
  jellED::SampleRecorder recorder(12000 * 5, "output.wav", 12000);

  std::cout << "Microphone sample rate: " << usbMicro.getSampleRate() << " Hz"
            << std::endl;
  std::cout << "Beat detection sample rate: "
            << (usbMicro.getSampleRate() / SIGNAL_DOWNSAMPLE_RATIO) << " Hz"
            << std::endl;
  ...
  int flushCounter = 0;
  int volumeUpdateCounter =
      0; // counts downsampled samples for periodic volume send
  ...
  while (true) {
    ...
    if (gotAudio) {
      ...
      uint32_t readAudioTimeUs = static_cast<uint32_t>(
          std::chrono::duration_cast<std::chrono::microseconds>(readAudioEnd -
                                                                readAudioStart)
              .count());
    ...
    if (gotAudio) {
      ...
      for (int i = 0; i < downsampledBuffer.num_samples; i++) {
        ...
        double sample = automaticGainControl.apply(gated);
        ...
        if (++volumeUpdateCounter >= VOLUME_UPDATE_INTERVAL) {
          ...
          // beatFlags intentionally not overwritten — a pending beat stays
          // pending
        }
      }
    }
